...
class GMM:
    ...
    def energy(self, z):
        ...
        with tf.compat.v1.variable_scope("GMM_energy"):
            ...
            v = tf.linalg.triangular_solve(self.L, tf.transpose(a=z_centered, perm=[1, 2, 0]))  # kli

            # log(det(Sigma)) = 2 * sum[log(diag(L))]
            log_det_sigma = 2.0 * tf.reduce_sum(input_tensor=tf.math.log(tf.linalg.diag_part(self.L)), axis=1)
